import React from 'react'
import icon from '../../assets/icon-upload.svg';
import Input from '../Input'

const Karyawan = ({ MunculForm, FungsiSpan, HandleOnSubmit, formKaryawan, handleChange, submitPhoto }) => {
  return (
    <div className={`flex flex-col w-md bg-white rounded-lg pb-5 shadow-lg 
    absolute top-64 right-1/12 ${MunculForm ? 'hidden' : 'block'}`}>
      <header className='flex rounded-lg flex-row justify-between p-2.5 bg-[#36C1FF]'>
        <h1 className='text-white font-semibold text-2xl font-public-sans'>Form Tambah Data Karyawan</h1>
        <span className='text-white bg-red-600 w-6 h-6 rounded-4xl justify-center text-center p-1 flex cursor-pointer'
          onClick={FungsiSpan}>
          <i className='fa fa-remove h-5 w-5'></i>
        </span>
      </header>

      {/* pastikan parent mengirim HandleOnSubmit (bukan "HandleOnSubnit") */}
      <form onSubmit={HandleOnSubmit} className='flex flex-col p-5'>
        <Input
          label="Nama Karyawan"
          name="name"
          placeholder="Input Nama Karyawan"
          type="text"
          required
          value={formKaryawan?.name ?? ''}
          onChange={handleChange}
        />
        <Input
          label="Email"
          name="email"
          placeholder="Input Email"
          type="email"
          required
          value={formKaryawan?.email ?? ''}
          onChange={handleChange}
        />
        <Input
          label="Divisi (ID)"
          name="divisionId"
          placeholder="Input ID Divisi"
          type="text"
          required
          value={formKaryawan?.divisionId ?? ''}
          onChange={handleChange}
        />
        <Input
          label="Jabatan"
          name="position"
          placeholder="Input Jabatan"
          type="text"
          required
          value={formKaryawan?.position ?? ''}
          onChange={handleChange}
        />
        <Input
          label="Tanggal Masuk"
          name="joinDate"
          placeholder="Pilih tanggal"
          type="date"
          required
          value={formKaryawan?.joinDate ?? ''}
          onChange={handleChange}
        />

        <div className='min-w-full h-28 flex flex-col justify-center items-center outline-1 outline-black'>
          <img src={icon} className='h-8 w-8' alt="upload" />
          <p>Upload Photo</p>
          <button type="button" className='bg-[#3EDD29] w-20 h-6 rounded-lg text-white cursor-pointer' onClick={submitPhoto}>select file</button>
        </div>

        <div className='flex flex-row gap-24 justify-center text-white mt-10'>
          <button type="reset" className='bg-[#FF5151] h-9 w-32 rounded-lg'>Hapus</button>
          <button className='bg-blue-500 h-9 w-32 rounded-lg' type='submit'>Simpan</button>
        </div>
      </form>
    </div>
  )
}

export default Karyawan
